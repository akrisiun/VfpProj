...
int _stdcall FindFoxFieldEx(int nFieldNo, Locator *pLoc, int nWorkarea)
{
	...
	if (nErrorNo = EVALUATE(vFieldName,aExeBuffer))
		return nErrorNo;

	LOCKHAND(vFieldName);
    nVarNti = _NameTableIndex(HANDTOPTR(vFieldName));
	UNLOCKHAND(vFieldName);
	FREEHAND(vFieldName);
...
int _stdcall StoreObjectRef(char *pName, NTI *pVarNti, Value *pObject)
{
	...
	if (nVarNti)
	{
		if (!_FindVar(nVarNti,-1,&lVar))
			return E_VARIABLENOTFOUND;

		// increment reference count by calling evaluate
		if (nErrorNo = EVALUATE(vObject,pName))
			return nErrorNo;

		if (nErrorNo = _Store(&lVar,pObject))
			return nErrorNo;

		if (nErrorNo = _ObjectRelease(pObject))
			return nErrorNo;
	}
	else
	{
		lVar.l_subs = 0;
		nVarNti = _NewVar(pName,&lVar,NV_PUBLIC);
		if (nVarNti < 0)
			return -nVarNti;
...
int _stdcall DBAppendRecords(int nWorkArea, unsigned int nRecords)
{
	...
	while (nRecords--)
	{
		if (nErrorNo = _DBAppend(nWorkArea,0)) // append blank records
			break;
	}
	UNLOCKW(nWorkArea);
	return nErrorNo;
...
char* _stdcall IntToStr(char *pString, int nNumber)
{
	...
	if (nNumber != 0)
	{
		do 
		{
			*pTmp++ = '0' + (unsigned int)nNumber % 10;
			(unsigned int)nNumber /= 10;
		}
		while (nNumber != 0);
