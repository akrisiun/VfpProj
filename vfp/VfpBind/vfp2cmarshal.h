...
#define END_ARRAYGET(nIter) \
			if ((nRetVal = STORE(r2,tmpValue)) == 0) \
				pAddress##nIter; \
			else \
				RAISEERROR(nRetVal); \
		} \
...
#define COLUMNGETLOCALS(pType) \
	...
	pColumn = HANDTOPTR(p2); \
	if (nErrorNo = FindFoxField(pColumn,&sFieldLoc,p3.ev_long)) \
		RAISEERROR(nErrorNo); \
	if ((nRetVal = RECCOUNTW(WORKAREA(sFieldLoc))) < p3.ev_long) \
	{ \
		if (nErrorNo = APPENDRECORDSW(WORKAREA(sFieldLoc),p3.ev_long-nRetVal)) \
			RAISEERROR(nErrorNo); \
	} \
	GOTOPW(WORKAREA(sFieldLoc));
...
#define END_COLUMNGET(nIter) \
		if ((nRetVal = REPLACE(sFieldLoc,tmpValue)) == 0) \
		{ \
			pAddress##nIter; \
			SKIPW(WORKAREA(sFieldLoc),1); \
		} \
		else \
			RAISEERROR(nRetVal); \
	}
...
#define BEGIN_COLUMNSET() \
	while(!EOFW(WORKAREA(sFieldLoc))) \
	{ \
		if ((nRetVal = LOAD(sFieldLoc,tmpValue)) == 0) \
		{
