...
namespace VfpLanguage
{
    ...
    internal sealed class PrgCommandFilter : IOleCommandTarget
    {
        private ICompletionSession _currentSession;

        public PrgCommandFilter(IWpfTextView textView, ICompletionBroker broker)
        {
            ...
            if (ErrorHandler.Succeeded(hresult))
            {
                if (pguidCmdGroup == VSConstants.VSStd2K && (VSConstants.VSStd2KCmdID)nCmdID == VSConstants.VSStd2KCmdID.TYPECHAR)
                {
                    ...
                    if (ch == ' ' || ch == ':' || char.IsSeparator(ch))
                        Cancel();
                    else if (ch == '.' && _currentSession != null)
                        Filter();
        ...
        bool StartSession(char? triggerChar = null)
        {
            if (_currentSession != null)
                return false;
