...
namespace MultiSelect
{
    ...
    public class SelectionMultiple : ISelectionStrategy
    {
        ...
        public bool Select(MultiSelectTreeViewItem item) { return Select(item, true); }
        public bool Select(MultiSelectTreeViewItem item, bool allowRemove)
        {
            if (allowRemove 
                && Mouse.LeftButton == MouseButtonState.Released
                && this.borderSelectionLogic.IsActive)
                this.borderSelectionLogic.CancelRectangle();
            ...
            if (IsControlKeyDown)
            {
                ...
                else
                {
                    ...
                    return SelectCore(item, allowRemove);
                }
            }
            else
            {
        ...
        public bool SelectCore(MultiSelectTreeViewItem item) { return SelectCore(item, true); }
        public bool SelectCore(MultiSelectTreeViewItem item, bool allowRemove = true)
        {
            if (IsControlKeyDown)
            {
                if (!treeView.SelectedItems.Contains(item.DataContext))
            ...
            else if (IsShiftKeyDown && treeView.SelectedItems.Count > 0)
            {
                ...
                List<object> newSelection = treeView.GetNodesToSelectBetween(shiftRootItem, item).Select(n => n.DataContext).ToList();
                ...
                if (allowRemove && !DeSelectItems(item, selectedItems, newSelection))
                    return false;
        ...
        bool DeSelectItems(MultiSelectTreeViewItem item, object[] selectedItems, IEnumerable<object> newSelection)
        {
        ...
        public bool DeSelectCore(MultiSelectTreeViewItem item)
        {
            //  DependencyPropertyKey IsMouseCapturedPropertyKey : System.Activities.Presentation, DragDropHelper ContentElement 
            if (treeView.IsMouseCaptured
                || Mouse.Captured != null) // || Thumb.IsDragging)
            {
                return true;
            }
